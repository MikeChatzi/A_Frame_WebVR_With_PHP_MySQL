...
<head>
	  ...
	  <script>
			...
			{
				if  (document.getElementById('myDiv0').style.display=='block') // an i forma einai emfanis tha klisei
				{
					
					document.getElementById('myDiv0').style.display='none';
				
				}
				else if (document.getElementById('myDiv0').style.display=='none') // an i forma einai kleisti
				{
						document.getElementById('myDiv0').style.display='block'; // emfanisi formas
				}
			}
	...
	<?php
		...
		if(in_array($truefiletype,$extensions)){
			...
			if (move_uploaded_file($filetmpname,$file_upload)) {
				$db="insert into `register` (userzname, fullname, email, password, userfile, textt) values('$nickz', '$users', '$emails', '$passwords', '$file_upload', '$captions')";
...
?>
	...
	<a-scene style="z-index:2">
		<a-assets>
				...
				<video id="TMR" autoplay loop muted="true" src="videos/Welcomeprj.mp4"> </video>
	...
	Account Form
	<button style="position:absolute; right:-0.5%;" onclick="showHideForm();">X</button>
	<form action="reception.php" method="post" enctype="multipart/form-data">
		...
		<div class="col-sm-10">
			...
			<button id="btnn" class="btn btn-dark mb-2" type="submit" name="create">Sign Up & Log In</button>
	...
	<button id="showLogin" onclick="showLoginForm();">Login</button>
	</div>
	...
	<form action="reception.php" method="post" enctype="multipart/form-data">
		...
		<div class="col-sm-10">
			...
			<button id="btnnn" class="btn btn-dark mb-2" type="submit" name="login">Log In</button>
	...
	<button id="showRegis" onclick="showRegistrationForm();">Return</button>
	</div>
	</a-entity>

	<!-- ROOM -->
	<a-entity geometry="primitive: box; width: 2.4; height: 1.2; depth: 0.02" material="src: #TMR" rotation="0 90 0" position="-8.64 3.25 9" play-video></a-entity>

	<a-entity geometry="primitive: cylinder" material="color: black" position="-8.64 3.9 9" scale="0.015 0.1 0.02"></a-entity>

	<a-entity geometry="primitive: box; width: 1.2; height: 0.6; depth: 0.02" material="color: black" rotation="0 0 0" position="-5 3.9 -1"></a-entity>
	...
	<a-entity geometry="primitive: plane; height: 20.5; width: 10.5" 
			...
			<a-entity 
					...
					description="Mike24: Welcome! I am Mike, the creator of this virtual space. I like anything that has to do with art! How about you? Fill up the form on the desk to create an account and let's connect!"
					...
					showtext>
							...
							<a-entity id="msg-1" geometry="primitive: plane; height: 0.22; width: auto" material="src: #merme; opacity: 0.65; side: double" style="z-index: 10" 
